...
class LearnableBias(nn.Module):
    def __init__(self, chn):
        ...
        self.bias = nn.Parameter(torch.zeros(1,chn,1,1), requires_grad=True)

    def forward(self, x):
        x = x + self.bias.expand(x.shape)
        return x
    ...
    def __init__(self, num_classes=1000):
        ...
        for i in range(len(stage_out_channel)):
            if i == 0:
                ...
                self.feature.append(ReActNetBlock(stage_out_channel[i-1], stage_out_channel[i], 2))
            else:
                self.feature.append(ReActNetBlock(stage_out_channel[i-1], stage_out_channel[i], 1))
